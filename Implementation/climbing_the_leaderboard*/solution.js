...
function leaderboard( alice, scores ) {
	let leaderboard = [];

	function add_score( score, start, end) {
		let middle_value = Math.floor(( start + end ) / 2 );
		if( middle_value === start ) {
			if( middle_value === 0 ) {
				leaderboard.splice( middle_value, 0, score);
				return middle_value + 1
			} else {
				leaderboard.splice( middle_value + 1, 0, score);
				return middle_value + 2
			}
		} else if( score > leaderboard[middle_value] ) {
			return add_score( score, start , middle_value )
		} else if( score < leaderboard[middle_value] ) {
			return add_score( score, middle_value, end);
		}
	}

	for ( score of scores ) {
		if ( leaderboard.indexOf(score) === -1 ) {
